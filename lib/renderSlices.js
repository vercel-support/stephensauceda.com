...
export default function(slices) {
  return slices.map((slice, i) => {
    switch (slice.slice_type) {
      ...
      case 'gist':
        return (
          <div style={{ marginBottom: '1em' }} key={i} dangerouslySetInnerHTML={{ __html: slice.primary.gist[0].text }} />
        )
      case 'image':
        return (
          <Fragment key={i}>
      ...
      case 'embed':
        return (
          <Fragment key={i}>
            <div className="embedContainer" dangerouslySetInnerHTML={{ __html: slice.primary.embed.html }} />
            <style>{`
              .embedContainer {
                position: relative;
                width: 100%;
